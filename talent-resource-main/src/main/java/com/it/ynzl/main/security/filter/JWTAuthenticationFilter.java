package com.it.ynzl.main.security.filter;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (token != null && token.startsWith(JWTConfig.tokenPrefix)) {
            ...
            if (JWTTokenUtils.isBlackList(token)) {
                ResponseUtils.responseJson(response, ResponseUtils.Failed("Token已失效,Token已进入黑名单"));
                return;
            }
            ...
            if (JWTTokenUtils.hasToken(token)) {
                ...
                if (JWTTokenUtils.isExpiration(expiration)) {
                    ...
                    if (JWTTokenUtils.isValid(validTime)) {
                        ...
                        // 加入黑名单
                        JWTTokenUtils.addBlackList(token);
                        ResponseUtils.responseJson(response, ResponseUtils.Failed("Token已过期 已超过刷新有效期"));
                        return;
                    }
                }
                ...
                if (sysUserDetails != null) {
                    ...
                    if (!StringUtils.equals(ip, sysUserDetails.getIp())) {
                        ...
                        // 加入黑名单
                        JWTTokenUtils.addBlackList(token);
                        ResponseUtils.responseJson(response,  ResponseUtils.Failed("Token已过期 可能存在IP伪造风险"));
                        return;
                    }
